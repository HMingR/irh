...
import top.imuster.common.core.dto.rabbitMq.SendDead2MQ;
...
public class GenerateSendMessageService {
    ...
    public void sendDeadMsg(SendDead2MQ msgEntry){
        try{
            rabbitTemplate.convertAndSend(RabbitMqConfig.EXCHANGE_TOPICS_INFORM, msgEntry.getType().getRoutingKey(), objectMapper.writeValueAsString(msgEntry));
        }catch (Exception e){
