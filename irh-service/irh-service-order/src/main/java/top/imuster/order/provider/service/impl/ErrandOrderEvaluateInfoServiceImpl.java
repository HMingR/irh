...
public class ErrandOrderEvaluateInfoServiceImpl extends BaseServiceImpl<ErrandOrderEvaluateInfo, Long> implements ErrandOrderEvaluateInfoService {
    ...
    public Message<ErrandOrderEvaluateInfo> getDetailById(Long id) {
        List<ErrandOrderEvaluateInfo> infos = errandOrderEvaluateInfoDao.selectEntryList(id);
    ...
    private void sendMessage(ErrandOrderEvaluateInfo evaluateInfo){
        ...
        userCenterDto.setContent(new StringBuffer().append("你接到的订单编号为: ").append(evaluateInfo.getErrandOrderCode()).append("的跑腿订单发布者已经进行了评价,评价结果为：").append(evaluateInfo.getEvaluate() == 1 ? "不满意" : "满意").append(",点击查看详情。如果对本次评价存在异议,可以在订单页面详情页面进行申诉").toString());
